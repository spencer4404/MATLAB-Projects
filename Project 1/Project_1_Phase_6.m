...
title('ECE 202 Project 1 Phase 6', ...
...
% E) I think the deviation will be the same, 
% as the function will still be plotted across 500ms, 
% as it was in the previous phases.

% F) As the function progresses past 500ms, 
% the main function that we are building just drops off, 
% as if it is just another term. 
% This makes sense as we do not have enough values to calculate
% the function through the whole time period. 
% We chose t = 0 because everything starts at 0 time, 
% as we can never really have negative time. The farther we get from t, 
% the more terms we need to build a complete function. 

% G) 27 terms seems to complete the function while also bringing 
% the avg deviation down significantly to 0.009. 
% This is higher than the original, 
% but that makes sense as we doubled the time 
% but havent fully doubled the terms.
